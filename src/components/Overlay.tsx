...
export default function Overlay({ camController }) {
  ...
  useAnimationFrame((e) => {
    if (camController.current) {
      ...
      if (lightRef.current) {
        lightRef.current.style.opacity = map(
          camController.current.polarAngle,
          0,
          Math.PI,
          0.4,
          0.8
  ...
  return (
    <>
      ...
      <div
        className={`wrapper fixed top-1/2 left-1/2 -translate-1/2 size-full z-40 pointer-events-none transition-opacity duration-500 ${
          activeProjectId ? "opacity-10 blur-xs" : "opacity-0 blur-xl"
        }`}
      >
        <img
          src=""
